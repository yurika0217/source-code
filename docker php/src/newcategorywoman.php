...
try {
  ...
  $stmt = $db->prepare("SELECT * FROM books WHERE category_id = 4
  -- AND flg=0
...
<div class="wrapper">
  ...
  while ($row = $stmt->fetch()) {
    ...
    echo '<p class="catedate">発売日：', $row['publish_date'], '</p>'; ?>
    <?php if ($row['stock'] > 0) : ?>
...
if ($max_page >= 1) {
  ...
  for ($i = 1; $i <= $max_page; $i++) {
    echo '<li class="page-item"><a href="newcategorywoman.php?page=' . $i . '">' . $i . '</a></li>';
  }
  echo '</ul></nav>';
}
